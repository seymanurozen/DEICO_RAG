...
class Ragify:
    ...
    def calculate_precision_k(self, chatbot_responses, reference_responses, k=1):
        ...
        for i in range(len(chatbot_responses)):
            ...
            # Precision should be intersection divided by total number of predicted tokens or `k`
            precision = len(intersection) / min(k,
                                                len(chatbot_tokens))  # Use `min(k, len(chatbot_tokens))` to handle cases where `k` is greater than the response length
